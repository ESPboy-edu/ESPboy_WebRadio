...
private:
    ...
    uint8_t cs_lcd;
    uint8_t cs_pin_mcp;                     // Pin where CS line is connected
    uint8_t dcoms_pin_mcp;                    // Pin where DCS line is connected
    uint8_t dreq_pin_mcp;                       // Pin where DREQ line is connected
...
protected:
    ...
    inline void await_data_request() const {
        while (!mcp->digitalRead(dreq_pin_mcp)) {
            yield();                        // Very short delay
        }
    }

    inline void control_mode_on() const {
        mcp->digitalWrite(cs_lcd, HIGH);
        SPI.beginTransaction(VS1053_SPI);   // Prevent other SPI users
        mcp->digitalWrite(dcoms_pin_mcp, HIGH);        // Bring slave in control mode
        mcp->digitalWrite(cs_pin_mcp, LOW);
    }

    inline void control_mode_off() const {
        mcp->digitalWrite(cs_pin_mcp, HIGH);         // End control mode
        SPI.endTransaction();               // Allow other SPI users
        mcp->digitalWrite(cs_lcd, LOW);
    }

    inline void data_mode_on() const {
        mcp->digitalWrite(cs_lcd, HIGH);
        SPI.beginTransaction(VS1053_SPI);   // Prevent other SPI users
        mcp->digitalWrite(cs_pin_mcp, HIGH);         // Bring slave in data mode
        mcp->digitalWrite(dcoms_pin_mcp, LOW);
    }

    inline void data_mode_off() const {
        mcp->digitalWrite(dcoms_pin_mcp, HIGH);        // End data mode
        SPI.endTransaction();               // Allow other SPI users
        mcp->digitalWrite(cs_lcd, LOW);
    }
    ...
    void sdi_send_buffer(uint8_t *data, uint32_t len);

    void sdi_send_fillers(uint32_t length);
...
public:
    ...
    VS1053(Adafruit_MCP23017 *mcp_pointer, uint8_t _cs_lcd, uint8_t _cs_pin_mcp, uint8_t _dcoms_pin_mcp, uint8_t _dreq_pin_mcp);
    ...
    void playChunk(uint8_t *data, uint32_t len);
    ...
    inline bool data_request() const {
        return (mcp->digitalRead(dreq_pin_mcp) == HIGH);
    }
